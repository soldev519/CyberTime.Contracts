...
beforeEach(async function () {
    ...
    ctfToken = await CTFToken.deploy(owner.address, owner.address, BigNumber.from("1000000000000000000"));
...
describe("CTFFarmCTFPool", function () {
    it("Should recieve the reward tokens successfully", async function () {
        ...
        await ctfFarm.claim(0);


        const ctfBalInMiddle = await ctfToken.balanceOf(owner.address);
        console.log("bal in middle",ctfBalInMiddle.toString() / 1e18)

        await ctfFarm.changeCTFPerBlock("0")
        ...
        const feeReceiverBal = await ctfToken.balanceOf(owner.address)
